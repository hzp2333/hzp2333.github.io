...
![当你和别人讨论便变量选择时，直接秒回，就是这种感觉！](/img/image-20240313212601245.png)

目前stata相应的命令五花八门——`asdoc`、`outreg2`、`logout`、`estout`。这里简单概括下各个命令常用在哪，又有何特点。
...
![asdoc描述性统计](/img/image-20240313180638960.png)

![asdoc回归统计](/img/image-20240313180712107.png)
...
![logout相关系数输出](/img/image-20240313193049158.png)

### outreg2:回归输出

表格输出，我选`outreg2`!表格美观，支持word，导出便利。

`ssc hot, n(10)`命令会展现stata热度前10的命令。`outreg`排名第一！

![stata外部命令排行榜](/img/image-20240313195502306.png)
...
`docx`系列命令来自[爬虫俱乐部](https://stata-club.github.io/)开发。以上大部分命令（asdoc不是）都意味着一个输出表格要单独占用一个word文件。`docx`改进后就可以重复使用一个文件。
...
stata17自带的输出命令,虽然用起来不怎么便利，但它似乎是唯一一个[^3]支持中文编码的命令。
...
![etable回归命令](/img/image-20240313212305154.png)
