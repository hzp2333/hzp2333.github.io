...
![一般赋权方法大赏（图源：http://www.huaxuejia.cn/ism/CESAISM/coupling-coordination.php）](/img/v2-ee0831674f8142ed9b128663cee0e25f_720w.png)
...
-   **提示1：正向指标和负向指标是不同的公式，需要区别处理**
-   **提示2：还有其他的规范化方法**

![图源：http://www.huaxuejia.cn/ism/CESAISM/coupling-coordination.php](/img/v2-4c54f74568dd7d885e89f2853a4a7cc8_720w.png)
...
![面板数据，五个指标衡量数字经济](/img/v2-092648dffbe03cc632867994f2ba3302_720w.png)
...
这个命令的**缺点**是，**mmx只提供了正向指标的线性变化，不能用于负向指标**。

也就是只有$\small X_{正指标}=\frac{X_{ij}-\min X_j }{\max X_j-\min X_j}$的变化，没有 负指标X负指标$\small X_{负指标}=\frac{\max X_j-X_{ij}}{\max X_j-\min X_j} $的变化。
...
![图源:《A mathematical theory of communication》（shannon,2001）](/img/v2-90b4404442340eb1a8ec8214270593d8_720w-17097108840205.png)
...
$$
p_{ij}=\frac{X_{ij}}{\sum_{\imath=1}^{n}X_{\imath}} 
$$
当$p_{ij}=0$时不适用这个式子，可以在归一化时直接把为0的数值进行补充。
...
![数字经济指标，个人找数据补到了2021年，额外进行了分组绘图。（左变异系数、右主成分法，总体排序基本是稳定的）](/img/v2-800d08e6f2b078fdffbae7b6e6a178bb_720w.png)
...
为了进一步形象化概括，我们可以用二维数据来展示pca分解的过程[^4]。

当数据标准化（标准正态分布）后，我们会得到呈现椭圆形的数据集。

![数据集](/img/v2-0a1081935783972114e5382164fcd41b_720w.png)

此时我们画一个平行于x轴的线，使它对应的方向上具有最大方差，此时这个**主成分** $PC_1$ 就是两者的线性组合。

$$
PC_1=a_{11}X_1+a_{12}X_2
$$
![分解](/img/v2-43d76bb22295d249e3e63901b526d799_720w.png)

同理，我们再画平行于y轴的线，同时注意到**y轴与x轴是垂直的，这样才能保证两个主成分之间是无关的**。

![y轴与x轴是垂直的，这样才能保证两个主成分之间是无关](/img/v2-f8c3cef3980efc9071966a0237e52c6f_720w.png)

有n个指标就是n维空间，也就有n个互相垂直的方向
...
-   $\boxed{维度}：$维度在数据中就是几个指标。有n个指标就是n维空间，也就有n个互相垂直的方向，也就是n个可以分解的主成分，但我们在操作中只需要部分贡献大的，所以取k个。**从n到k，这个过程就是** $\boxed{降维} $。
...
-   stata的pca命令自带数据规范化，不需要额外预处理操作
-   分析数据的负向指标需要自己正向化：$\small 1-x_{负向指标} 、\small \frac{1}{x_{负向指标}}......$
-   主成分法给出的是主成分$PC_k$ ,也就是n个指标分解成了k个主成分，然后加总给出总分。原始指标没有权重。
...
![数据源就是前文附件的excel:《数字经济、创业活跃度与高质量发展——来自中国城市的经验证据》](/img/v2-9b3990c2463c959aecd85e192bdc9eae_720w.png)

**一般要求特征值要大于1**，小于1的话说明特征变量的解释能力还不如原变量。 若三个变量特征值均大于1，则要提取的主成分有三个。图中大于1的只有一个，说明我们只需要提取一个主成分。
...
![碎石图，选出大于1的对应主成分](/img/v2-8d62d864fdb0bf77c77db70a0b8725e9_720w.png)
...
![结果](/img/v2-e7f705e0427ff55aba8d0bee20fd5e74_720w-170971169316814.png)
...
假设我们有四个指标，四个指标特征值都大于1，也就是我们四个主成分都可以用，也就可以通过**得分图**来聚类。
...
![得分分析](/img/v2-f00d413628e66197bc1997e8f9f707d0_720w.png)

对于得分图来说，**距离越近越相似**，由于我们选择了参数factors(4)，那么四个主成分两两组合，就会产生 $C_4^2=6$个得分图。我们使用的是面板数据，我们按照的地域标签，确实可以看出**不同年份的同一个城市（北京、重庆）**总是聚类在一起。
...
由于我们选择了参数factors(4)，那么四个主成分两两组合，就会产生$C_4^2=6$个因子载荷图  

它反映了不同组成分下指标的聚类程度。

![因为是对称的，所以只看一侧就行，也就是有6个图](/img/v2-58941bc6ef0fa74e3ea3a19c761ac97b_720w.png)

因为是对称的，所以只看一侧就行，也就是有6个图

![我做过的一个指标得分，指标数量有30个，主成分取了12个，最终结果没有太大变化](/img/v2-2d9bffe1c2d1526d8972ab6d0b2d6fdf_720w.png)

我做过的一个指标得分，指标数量有30个，主成分取了12个，最终结果没有太大变化
