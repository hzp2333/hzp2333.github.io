...
{{< admonition type=bug  title="如果上面的方式失效" open=false >}}

Stata 对中文的兼容 非常糟糕。因此有可能使用了上面的方法，英文也都是宋体，或者中文依旧是黑体。这是因为系统没有真正识别出系统的字体。这时候如下图所示，先在一个图中 `编辑-首选项-字体` 中把图片各个部分编辑成自己想要的字体。然后之后运行就可以顺利识别。

{{< /admonition >}}

![如图](/img/Stata图片中英文字体设置.zh-cn-1784540366747.webp)
