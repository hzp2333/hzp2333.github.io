...
>
> 在整理的过程中我反复感受到开源的力量，所有的处理都有 R 语言原生的技巧，但总是时不时冒出各种包让你更省事，所以，成为调包侠吧！
...
![数据库$列名是指定列变量的方式，Rstudio的很多功能还是挺好用的](/img/image-20240421160048211.png)
...
新建变量 x ,截取变量 var 第一个位置开始，长度为4的字符串
...
虽然R语言自带 merge ,和 stata 的 merge 类似，但功能更弱且只能进行一对一匹配，且大部分时候还要求一对一，这里推荐使用`dplyr`包。

![横向](/img/d038fef03d9f614a963b84b7793d183e_720w.webp)

![纵向](/img/8b906bd28d5de5b0eafc5043d7e0ff43_720w.webp)
...
由于stata一次性只能操作一张表，所以都是在一张表上增减，但是 R 语言支持多张表操控，所以实际上是通过**截取合适数据形成一张新的表**来达成筛选目的。
...
![散点图加拟合线](/img/image-20240421181329822.png)

![切片以后](/img/image-20240421183000092.png)
...
![内部再分组的直方图](/img/ggplot2-right-barseg-gss01-1.png)
...
![midwest是自带数据库](/img/image-20240421201251655.png)
...
![wordcloud2包自带的数据库](/img/image-20240421200852275.png)
...
![相关系数热力图](/img/image-20240421203317326.png)
...
![气泡图](/img/image-20240421220309549.png)
...
![2*2的分割](/img/image-20240421225144167.png)
...
![输出效果](/img/image-20240421233614572.png)
...
![bruceR的相关性分析，第一次使用时我惊呆了！](/img/image-20240421233527153.png)

### 回归和 word 输出

#### 回归

- `robust="HC1"`:稳健性模式设置，HC1 就是 stata 中的`reg y x,r`
...
![回归结果](/img/image-20240422003146640.png)
